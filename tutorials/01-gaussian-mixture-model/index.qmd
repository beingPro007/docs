...
We use a `Gibbs` sampler that combines a [particle Gibbs]({{< meta particle-gibbs-doc >}})
sampler for the discrete parameters (assignments $k$) and a Hamiltonion Monte Carlo sampler for the continuous parameters ($\mu$ and $w$).
...
will run sequentially, and you may notice a warning. For more information, see [the Turing documentation on sampling multiple chains.]({{< turing-doc-sampling-multiple-chains >}})
...
For more information see the [Stan documentation]({{< stan-doc-identifying-mixture-models >}})
. This is because it's possible for either model parameter $\mu_k$ to be assigned to either of the corresponding true means, and this assignment need not be consistent between chains.
...
`Bijectors.jl` [provides]({{< bijectors-ordered-bijector >}})
 an easy transformation (`ordered()`) for this purpose:
...
Where we sum the components with `logsumexp` from the [`LogExpFunctions.jl` package]({{< logexpfunctions-doc >}}).
...
In fact, `Distributions.jl` provides [many convenient constructors]({{< distributions-mixture-doc >}}) for mixture models, allowing further simplification in common special cases.
...
For details on the mathematics here, see [the Stan documentation on latent discrete parameters]({{< stan-latent-discrete-doc >}}).
```{julia}
#| output: false
