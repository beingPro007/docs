...
See the You can refer to the [documentation of the `condition` syntax]({{< meta condition-doc >}}).
 in `DynamicPPL.jl` for more details.
...
In this example, we use a [Hamiltonian Monte Carlo]({{< meta hmc-doc >}}) sampler to draw these samples.
...
The `sample` function and common keyword arguments are explained more extensively in the documentation of [AbstractMCMC.jl]({{< meta abstractmcmc-doc >}}).
